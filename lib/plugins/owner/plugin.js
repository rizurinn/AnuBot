...
async function pluginManager(m, bill) {
  const { Nreply, args, command, prefix } = bill;
...
${prefix}${command} reloadall - Me-reload semua plugin
  ...
  switch (action) {
    ...
    case 'get':
      if (args.length < 2) {
        return await Nreply(`Gunakan: ${prefix}${command} get [nama_plugin]`);
      }
      return await getPlugin(args[1], m, bill);
    
    case 'add':
      if (args.length < 3 || !m.quoted) {
        return await Nreply(`Gunakan: ${prefix}${command} add [nama_file] [kategori] (reply ke kode plugin)`);
      }
      return await addPlugin(args[1], args[2], m.quoted.text, m, bill);
    
    case 'sf':
      if (args.length < 2 || !m.quoted) {
        return await Nreply(`Gunakan: ${prefix}${command} save [nama_plugin] (reply ke kode plugin yang baru)`);
      }
      return await savePlugin(args[1], m.quoted.text, m, bill);
    
    case 'del':
      if (args.length < 2) {
        return await Nreply(`Gunakan: ${prefix}${command} delete [nama_plugin]`);
      }
      return await deletePlugin(args[1], m, bill);
      
    case 'reload':
      if (args.length < 2) {
        return await Nreply(`Gunakan: ${prefix}${command} reload [nama_plugin]`);
      }
    ...
    default:
      return await Nreply(`Aksi tidak dikenal: ${action}`);
  }
}
...
async function getPlugin(pluginName, m, bill) {
  const { Nreply } = bill;
  const plugin = findPlugin(pluginName, bill);
  
  if (!plugin) {
    return await Nreply(`❌ Plugin '${pluginName}' tidak ditemukan`);
  }
  ...
  try {
    ...
    await Nreply(`${pluginCode}`);
  } catch (error) {
    await Nreply(`❌ Error saat mengambil source code plugin: ${error.message}`);
  }
}
...
async function addPlugin(fileName, category, pluginCode, m, bill) {
  const { Nreply, prefix, command } = bill;
  ...
  try {
    ...
    if (fs.existsSync(pluginPath)) {
      return await Nreply(`❌ Plugin dengan nama file '${fileName}' sudah ada di kategori '${category}'`);
    }
    ...
    if (!hasCommand && !hasBefore) {
      return await Nreply(`❌ Kode plugin tidak valid. Plugin harus memiliki properti command atau fungsi before.`);
    }
    ...
    try {
      ...
      if (!isValid) {
        ...
        return await Nreply(`❌ Plugin tidak valid. Pastikan format sesuai dan memiliki properti command atau fungsi before.`);
      }
    } catch (loadError) {
      fs.unlinkSync(pluginPath);
      return await Nreply(`❌ Error saat memuat plugin: ${loadError.message}`);
    }
    ...
    await Nreply(`✅ Plugin '${pluginName}' (${pluginType}) berhasil dibuat di \`plugins/${category.toLowerCase()}/${fileName}\``);
  } catch (error) {
    await Nreply(`❌ Error saat membuat plugin: ${error.message}`);
  }
}
 ...
 */
async function savePlugin(pluginName, newCode, m, bill) {
  const { Nreply } = bill;
  const plugin = findPlugin(pluginName, bill);
  ...
  try {
    ...
    try {
      ...
      if (!isValid) {
        ...
        return await Nreply(`❌ Plugin baru tidak valid. Plugin asli telah dikembalikan.`);
      }
      ...
      fs.copyFileSync(backupPath, pluginPath);
      fs.unlinkSync(backupPath);
      return await Nreply(`❌ Error saat memuat plugin baru: ${loadError.message}. Plugin asli telah dikembalikan.`);
    }
    ...
    await Nreply(`✅ Plugin '${pluginName}' (${pluginType}) berhasil disimpan di \`${relativePath}\``);
    
  } catch (error) {
    await Nreply(`❌ Error saat menyimpan plugin: ${error.message}`);
  }
}
...
async function deletePlugin(pluginName, m, bill) {
  ...
  try {
    ...
    await Nreply(`✅ Plugin '${pluginName}' (${pluginType}) berhasil dihapus dari ${relativePath}`);
  } catch (error) {
    await Nreply(`❌ Error saat menghapus plugin: ${error.message}`);
  }
}
...
async function reloadPlugin(pluginName, m, bill) {
  ...
  try {
    ...
    if (result) {
      await Nreply(`✅ Plugin '${pluginName}' berhasil di-reload`);
    } else {
      await Nreply(`❌ Plugin '${pluginName}' tidak ditemukan atau gagal di-reload`);
    }
  } catch (error) {
    await Nreply(`❌ Error saat me-reload plugin: ${error.message}`);
  }
}
 ...
 * Reload all plugins
 */
async function reloadAllPlugins(m, bill) {
  const { Nreply } = bill;
  try {
