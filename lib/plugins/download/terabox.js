...
async function handler(m, bill) {
    const { args, rinn, msg } = bill;
    const url = args[0];
    
    if (!url) return m.reply('Tambahkan URL Terabox yang ingin diunduh!');
    
    try {
        m.reply('🔍 Sedang memproses tautan Terabox...');
        
        const response = await terabox.download(url);
        
        if (response.status !== 'success') {
            return m.reply(response.message || 'Gagal mengunduh file');
        }
        
        const fileData = response.result.structure;
        
        if (!fileData.direct_link) {
            return m.reply('Tidak dapat menemukan tautan unduhan.');
        }
...
`;
        ...
        m.reply('Terjadi kesalahan saat mengunduh file dari Terabox');
    }
}

handler.command = ['terabox', 'teraboxdl', 'trboxdl'];
