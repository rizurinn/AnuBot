...
let handler = async (m, { rinn, Nreply, text, args, command }) => {
    if (!text && !m.quoted) {
        return Nreply(`*Penggunaan:*
...
Anda juga bisa membalas pesan yang berisi link YouTube.`);
    ...
    if (m.quoted && !text) {
        ...
        if (match) {
            ...
            return Nreply('❌ Tidak menemukan link YouTube di pesan yang dibalas!');
        }
    }
    ...
    if (!ytdown.isUrl(url)) {
        return Nreply('❌ URL tidak valid! Pastikan Anda memasukkan link YouTube yang benar.');
    }
    
    const youtubeId = ytdown.youtube(url);
    if (!youtubeId) {
        return Nreply('❌ Link YouTube tidak valid!');
    }
    ...
    // Loading message
    const waitMsg = await Nreply(`🔄 *Sedang memproses...*
*Link:* ${url}
*Format:* ${format}
...
⏳ Harap tunggu, proses ini mungkin memakan waktu beberapa saat...`);
    ...
    try {
        ...
        if (isAudio) {
            ...
            if (result.error) {
                return Nreply(`❌ *Error:* ${result.error}`);
            }
            ...
            if (!result.status) {
                return Nreply(`❌ *Error:* ${result.metadata?.message || 'Terjadi kesalahan saat mengunduh video'}`);
            }
...
⏬ *Mengirim file${isAudio ? ' audio' : ' video'}...*`,
        ...
        return Nreply(`❌ *Terjadi kesalahan!*
