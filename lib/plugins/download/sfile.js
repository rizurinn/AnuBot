...
let handler = async (m, { rinn, Nreply, args, prefix, command }) => {
  if (!args[0]) {
    return Nreply(`Silahkan masukkan URL Sfile\nContoh: ${prefix + command} https://sfile.mobi/87GP8jpuRar`);
  }
  
  if (!args[0].match(/sfile\.mobi/i)) {
    return Nreply('URL tidak valid! Pastikan URL dari sfile.mobi');
  }
  ...
  try {
    ...
    Nreply(`Terjadi kesalahan: ${error.message}`);
  }
};
