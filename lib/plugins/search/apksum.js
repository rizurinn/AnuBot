...
const handler = async (m, {
    ...
    if (!args[0]) {
        return m.reply('Metode yang tersedia\n\n- Search <Query>\n- Download <url halamam>');
    }
    if (args[0] === 'search') {
    ...
    if (args[0] === 'download') {
        const url = args[1];
        if (!url) {
            return m.reply('Masukkan URL aplikasi yang ingin diambil downloadnya. Contoh penggunaan: apksum download https://m.apksum.com/download/com.mojang.minecraftpe_1.21.60.25_free');
        }
        ...
        try {
            ...
            if (downloadLinks.length === 0) {
                return m.reply("Tidak ditemukan link download untuk aplikasi ini.");
            }
