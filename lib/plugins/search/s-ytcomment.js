...
let handler = async (m, { Nreply, text }) => {
    if (!text) return Nreply('[EXAMPLE!]\n\n- .sytc url-video-yt|query')

    let [url, query] = text.split('|').map(v => v.trim())

    if (!url) return Nreply('[EXAMPLE!]\n\n- .sytc url-video-yt|query')
    if (!query) query = ''
    let idVideo = ngambilID(url)
    if (!idVideo) return Nreply('URL YouTube tidak valid.')
    ...
    try {
        ...
        if (!komen || komen.length === 0) return Nreply('gada yg cocok.')
        ...
        await Nreply(`Hasil komentar:\n\n${hasil}`)
    } catch (error) {
        await Nreply('Error saat mengambil komentar.')
    }
}
