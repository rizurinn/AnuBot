...
const handler = async (m, {
    ...
    try {
        ...
        if (stickerPacks.length === 0) {
            return m.reply('Tidak ditemukan sticker packs untuk pencarian ini.');
        }
        ...
        if (stickers.length === 0) {
            return m.reply('Tidak ada sticker yang ditemukan pada pack ini.');
        }

        const jumlah = stickers.length;
        const jmlah = Math.min(count, jumlah);

        m.reply(`Ditemukan ${jumlah} sticker. Mengirimkan ${jmlah} sticker...`);
