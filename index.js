...
await rinn.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
return waMessage
}
...
rinn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
    ...
    let m;
    try {
        m = await rinn.sendMessage(jid, message, { ...opt, ...options });
    } catch (e) {
        console.error(e);
        m = null;
    } finally {
        if (!m) m = await rinn.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options });
        return m;
    }
}
...
rinn.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
    ...
    await rinn.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted });
    return buffer;
};
...
rinn.sendText = (jid, text, quoted = '', options) => rinn.sendMessage(jid, { text: text, ...options }, { quoted })

rinn.sendTextWithMentions = async (jid, text, quoted, options = {}) => rinn.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
return rinn;
}
