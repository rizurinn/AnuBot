...
exports.smsg = (rinn, m, store) => {
	...
	if (m.key) {
		...
		m.sender = rinn.decodeJid(m.fromMe && rinn.user.id || m.participant || m.key.participant || m.chat || '');
		if (m.isGroup) m.participant = rinn.decodeJid(m.key.participant) || '';
	}
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = rinn.decodeJid(m.msg.contextInfo.participant);
			m.quoted.fromMe = m.quoted.sender === rinn.decodeJid(rinn.user.id);
			...
			m.quoted.delete = () => rinn.sendMessage(m.quoted.chat, {
				delete: vM.key
			});
			m.quoted.copyNForward = (jid, forceForward = false, options = {}) => rinn.copyNForward(jid, vM, forceForward, options);
			m.quoted.download = () => rinn.downloadMediaMessage(m.quoted);
		}
	}
	if (m.msg.url) m.download = () => rinn.downloadMediaMessage(m.msg);
	...
	m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? rinn.sendMedia(chatId, text, 'file', '', m, {
		...options
	}) : rinn.sendText(chatId, text, m, {
		...options
	});
	m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
	m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => rinn.copyNForward(jid, m, forceForward, options);
	rinn.appenTextMessage = async (text, chatUpdate) => {
		...
		messages.key.fromMe = areJidsSameUser(m.sender, rinn.user.id);
		...
		rinn.ev.emit('messages.upsert', msg);
	};
	return m;
};
